...
def shot_sweep(n, t, fixed_probes, shot_grid, seed_instance,
               n_trials=32, master_seed=99, n_jobs=None):
    if n_jobs is None:
    ...
    for shots in tqdm(shot_grid, desc="shot sweep"):
        ...
        results = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(_trial)(n, t, fixed_probes, shots, seed_instance,
...
def plot_sweep(records, fixed_probes, n, out_path):
    ...
    if fit is not None:
        axes[0].plot(x, fit, "--", color="grey", alpha=0.7,
                     label=rf"$C/\sqrt{{\nu}}$  (C={C:.3f})")
    axes[0].set_xlabel(r"shots per probe $\nu$")
    axes[0].set_ylabel(r"$\|\hat H - H\|_{\mathrm{op}}$")
    axes[0].set_title(f"TFIM, n={n}, probes={fixed_probes}")
    ...
    if fit is not None:
        axes[1].plot(x, fit, "--", color="grey", alpha=0.7,
                     label=rf"$C/\sqrt{{\nu}}$  (C={C:.3f})")
    axes[1].set_xlabel(r"shots per probe $\nu$")
    axes[1].set_ylabel(r"mean $|\hat h_i - h_i|$")
    axes[1].set_title(f"TFIM, n={n}, probes={fixed_probes}")
...
if __name__ == "__main__":
    ...
    plot_sweep(records, fixed_probes=fixed_probes, n=n,
               out_path=here / "tfim_shot_sweep.pdf")
